...
# ══════════════════════════════════════════════════════════════════════════════
#
# ORDER MATTERS. More specific rules must appear before broader catch-alls.
# When adding rules, place them above any catch-all that could shadow them.
# Slug format: lowercase, spaces/hyphens/dots → "." (e.g. "kenya.ai.strategy")
# Patterns match against the slug, not the original filename.
DOCUMENT_TYPE_RULES = [
    ...
    # Improved rule to catch CBK_34th.pdf and similar numbered MPC reports
    ...
    # FIX 7: these patterns must appear BEFORE the generic kcse.*statistics rule
    # because the slug may have "statistics" before "kcse" (e.g. statistics.report.knec.kcse)
    (r"statistics.*knec|statistics.*kcse|report.*knec.*kcse", "statistics_report"),
    ...
    # FIX 7: kenya.*ai.*strategy and slug-order variants must appear BEFORE the
    # generic digital.*economy research_report rule further down, because the slug
    # for "2025-Strategy-GOK-Kenya-AI.pdf" is "2025.strategy.gok.kenya.ai" —
    # "strategy" precedes "kenya.ai", so only a flexible pattern catches it.
    (r"kenya.*ai.*strategy|strategy.*gok.*kenya.*ai|gok.*kenya.*ai", "strategic_plan"),
    # FIX 7: digital.*economy.*strategy must precede the digital.*economy.*2019 rule
    # so "2024-Kenya-Digital-Economy-Strategy.pdf" → strategic_plan, not research_report
