...
# Category → chunk_size mapping (from doc_type_taxonomy.py)
CHUNK_SIZE_MAP = {
    1: 512,
    2: 2048,      # tables_only — can be larger
    3: 1024,
    4: 768,
    5: 512,
    6: 512,
    7: 1024,
    8: 1536,      # hybrid — merges text + tables
    0: 350,       # default
}

# Category → chunk_overlap mapping (from doc_type_taxonomy.py)
CHUNK_OVERLAP_MAP = {
    1: 128,       # 25% of 512
    2: 0,         # tables — no overlap
    3: 204,       # 20% of 1024
    4: 154,       # 20% of 768
    5: 128,       # 25% of 512
    6: 128,       # 25% of 512
...
def build_document_entry(row: pd.Series, hard_reasons: list[str], soft_reasons: list[str]) -> dict:
    ...
    chunk_size and chunk_overlap are derived from the category field using
    CHUNK_SIZE_MAP and CHUNK_OVERLAP_MAP.
    """
    ...
    # Derive chunk_size and chunk_overlap from category
    category = _parse_int_col(row.get("category", 0))
    chunk_size = CHUNK_SIZE_MAP.get(category, 350)
    chunk_overlap = CHUNK_OVERLAP_MAP.get(category, 88)
    ...
    entry: dict[str, Any] = {
        ...
        "category":                 category,
        "chunking_strategy":        str(row.get("chunk_strategy", "narrative")).strip(),
...
def write_config(documents: list[dict], output_path: Path, agent_filter: str | None) -> None:
    ...
    yaml.dump(
        {"pipeline": config["pipeline"], "meta": config["meta"], "documents": config["documents"]},
        stream,
        Dumper=dumper,
        allow_unicode=True,
        sort_keys=False,
